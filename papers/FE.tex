...
In the model the role of Central Bank is to fix the Government's Bonds interest rate according a Taylor rule $i = \pi + \alpha_1 (\pi - \pi^*) + \alpha_2 (u - u^*)$, where $\pi$ is the inflation rate, $u$ is the capacity utilization measured as the fraction of capital goods used in production, and starred variables are the targets.
...
Capital goods are characterized by their productivity $\beta$ and the minimum skill level required to operate them $\sigma$. Each period they have a fixed probability to broke and disappear from the model, equal to $<N>^{-1}$, where $<N>$ is the expected life of the machinery.
