...
Noted $s_{t-1}$ the number of goods sold in the previous period, each Firm sets the desired production as $\mathbf{c}^*_t = \rho_\mathbf{C}(1+g-\psi)s_{t-1}$, where $g$ is the GDP growth rate.
...
The request for loans is set to cover salaries and investments, keeping in account revenues, i.e. $l_t^* = \max(\mathbf{d}_{t-1} - \rho_\mathcal{F} w_t^*, \mathbf{d}_{t-1} + (1+\psi){p_\mathbf{C}}_t-1 (1+g-\psi)s_{t-1} - \rho_\mathcal{F} (w_t^* + i_t^*), 0)$.

The markup is set as $\mu_t = \mu_{t-1}(1 + \Theta \frac{s_{t-1}}{\mathbf{c}_t})$ and the price ${p_\mathbf{C}}_t = (1+\mu_t+\tau_\mathbf{C})\frac{w_t}{\mathbf{c}_t}$ after the production \parencite[like in][]{caiani2016}, where $w$ is the sum of the wages paid by the firm and $\tau_\mathbf{C}$ is the VAT rate.
...
Firms can fire workers only when profits turn negative. In that case, Firms fire workers until the number of workers reaches $u^* \mathbf{k}$. Additionally, in the case the Firm is not able to pay a worker, they is fired. In both cases, Firms start to fire those with lower skills.
...
\begin{tabularx}{\linewidth}{lXll}
	...
	$\Sigma$           & Relative skill variation                                   &       &        \\
	                   & Distribution parameters to assign initial skill level      &       &        \\
	$\rho_\mathcal{H}$ & Deposits to consumption ratio                              &       &        \\
	                   & Distribution parameters for marginal propensity to consume &       &        \\
	$\rho_\mathbf{C}$  & Production over expected sales                             &       &        \\
	$\rho_\mathcal{F}$ & Liquidity over expected wages and investiment ratio        &       &        \\
...
%The next steps are to implement the model in a suitable programming language, define a strategy to initialize the model and list the stylized facts the model should reproduce and quantify some of them relying on real data.

%By the time of the conference, I'm confident that I will be able to present an implemented version of the model which can reproduce some stylized facts at least in a qualitative way.
