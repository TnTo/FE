...
function compute_u(m::Model, t::Int)::Float
    N = mapsum(f -> mapsum(g -> β(m, f, g), filter(k -> k.operator !== nothing, f.K)), values(m.states[t].FCs)) + mapsum(f -> mapsum(g -> β(m, f, g), filter(k -> k.operator !== nothing, f.K)), values(m.states[t].FKs))
    D = mapsum(f -> b(m, f), values(m.states[t].FCs)) + mapsum(f -> b(m, f), values(m.states[t].FKs))
    if D == 0
        return 1
    else
        return N / D
...
function set_target!(f::ConsumptionFirm, f1::ConsumptionFirm, Es::Int, s::State, s1::State, m::Model)
    ...
    if length(f.K) > 0 && length(f1.employees) > 0
        w_ = max(f1.w, m.p.p0, ceil(Int, f.c_ / mean(map(k -> β(m, f, k), f.K)) * (f1.w / length(f1.employees))))
    else
        w_ = max(f1.w, m.p.p0)
    end
    if length(f.K) > 0
        i_ = ceil(Int, (1 + s.stats.ψ)^(1 / 12) * mean(map(k -> ((k.p * m.p.NK / (m.p.NK - k.age + 1)) / β(m, f, k)), f.K)) * f.Δb_)
    else
...
function set_target!(f::CapitalFirm, f1::CapitalFirm, Es::Int, s::State, s1::State, m::Model)
    ...
    f.Δb_ = b_ - b0
    f.k_ = max(0, max(1, ceil(Int, m.p.ρK * Es + f.Δb_ / (f1.β * m.p.k))) - length(f.inv))
    avg_wQ = wQ(f, s, s1, m)
    f.q_ = count(r -> r.operator !== nothing, f1.Q) + floor(Int, m.p.ρQ * (f1.p * f1.s - f1.w) / avg_wQ)
    if length(f.K) > 0 && length(f1.employees) > 0
        w_ = max(f1.w, m.p.p0, ceil(Int, f.k_ / mean(map(k -> β(m, f, k), f.K)) * (f1.w / length(f1.employees)) + f.q_ * avg_wQ))
    else
        w_ = max(f1.w, m.p.p0)
    end
...
function stepI!(state::State, state1::State, m::Model)
    ...
    for (id, f) in state.FCs
        f.c = floor(Int, mapsum(k -> β(m, f, k), filter(k -> k.operator !== nothing, f.K)))
        if f.c == 0
    ...
    for (id, f) in state.FKs
        f.k = floor(mapsum(k -> β(m, f, k), filter(k -> k.operator !== nothing, f.K)))
        if f.k == 0
            f1 = get_f_by_id(f.id, state1)
            if f1 === nothing
                f.p = ceil(Int, (1 + f.μ) * Ewσ(f.σ, state) / (f.β * m.p.k))
            else
                f.p = ceil(Int, f1.p * (1 + f.μ) / (1 + f1.μ))
            end
        else
