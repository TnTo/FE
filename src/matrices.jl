...
function compute_balance_sheet(m::Model, t::Int)::NamedArray{Int}
    ...
    balance[:L, :FC] = -mapsum(l -> l.principal, vcat(map(a -> a.L, values(s.FCs))...))
    balance[:L, :FK] = -mapsum(l -> l.principal, vcat(map(a -> a.L, values(s.FKs))...))
    balance[:L, :B] = s.B.L
...
function compute_flow_matrix(m::Model, t::Int)::NamedArray{Int}
    ...
    flow[:C, :H] = -mapsum(a -> a.nc - a.tC, values(s.Hs))
    flow[:C, :FC] = mapsum(a -> a.s * a.pF, values(s.FCs))
    flow[:C, :G] = -s.G.nC
    flow[:C, :Tot] = sum(flow[:C, :])
    flow[:I, :FC] = -mapsum(a -> a.i, values(s.FCs))
    flow[:I, :FK] = mapsum(a -> a.y, values(s.FKs))
    flow[:I, :Tot] = sum(flow[:I, :])
    flow[:W, :H] = mapsum(a -> a.wH + a.tW, values(s.Hs))
    flow[:W, :FC] = -mapsum(a -> a.wF, values(s.FCs))
    flow[:W, :FK] = -mapsum(a -> a.wF, values(s.FKs))
    flow[:W, :Tot] = sum(flow[:W, :])
    flow[:T, :H] = -mapsum(a -> a.tC + a.tW + a.tS + a.tI, values(s.Hs))
    flow[:T, :G] = s.G.T
    ...
    flow[:rS, :H] = floor(Int, s.B.rS * mapsum(a -> a.S, values(s.Hs)))
